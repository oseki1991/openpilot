...
class CarInterfaceBase(ABC):
    ...
    def create_common_events(self, cs_out, extra_gears=None, pcm_enable=True):
        ...
        if self.dragonconf.dpGearCheck and cs_out.gearShifter not in (
                GearShifter.drive, GearShifter.sport, GearShifter.eco, GearShifter.low) and (extra_gears is None or
...
class CarStateBase(ABC):
    ...
    def update_speed_kf(self, v_ego_raw):
        if abs(v_ego_raw - self.v_ego_kf.x[0][
            0]) > 2.0:  # Prevent large accelerations when car starts at non zero speed
